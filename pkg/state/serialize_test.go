...
// fill puts every entry of es into a fresh Memory, in the order given.
func fill(es []entry) *Memory {
	m := NewMemory()
	for _, e := range es {
		m.Put([]byte(e.key), []byte(e.value))
	}
	return m
}
...
func TestSerializeRoundTrips(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := WriteTo(fill(tt.entries), &buf); err != nil {
			...
			slices.SortFunc(want, func(a, b entry) int { return bytes.Compare([]byte(a.key), []byte(b.key)) })
...
func TestWriteToIsIndependentOfInsertionOrder(t *testing.T) {
	...
	for _, o := range orders {
		var buf bytes.Buffer
		if err := WriteTo(fill(o.entries), &buf); err != nil {
...
func TestWriteToWritesTheDocumentedBytes(t *testing.T) {
	var buf bytes.Buffer
	if err := WriteTo(fill([]entry{{key: "ab", value: "xyz"}}), &buf); err != nil {
		t.Fatalf("WriteTo: %v", err)
	}
...
func TestReadFromRefusesANonEmptyState(t *testing.T) {
	var buf bytes.Buffer
	if err := WriteTo(fill([]entry{{key: "a", value: "1"}}), &buf); err != nil {
		t.Fatalf("WriteTo: %v", err)
	}

	dirty := fill([]entry{{key: "b", value: "2"}})
...
func TestReadFromRejectsATruncatedStream(t *testing.T) {
	var buf bytes.Buffer
	if err := WriteTo(fill([]entry{{key: "ab", value: "xyz"}, {key: "cd", value: "w"}}), &buf); err != nil {
		t.Fatalf("WriteTo: %v", err)
	}
	full := buf.Bytes()
	...
	for n := 0; n < len(full); n++ {
		...
		if err := ReadFrom(restored, bytes.NewReader(full[:n])); err == nil {
			t.Errorf("ReadFrom over the first %d of %d bytes returned nil, restoring %d entries", n, len(full), restored.Len())
		}
	}
...
func TestWriteToReportsAFailedWrite(t *testing.T) {
	s := fill([]entry{{key: "ab", value: "xyz"}, {key: "cd", value: "w"}})
...
func TestSerializeReportsABackendError(t *testing.T) {
	failing := &iterateFailsState{inner: fill([]entry{{key: "a", value: "1"}})}
